...
impl RequestParser {
        ...
        if !self.buf.is_empty() {
            self.tokens.push(Token::Text(
                String::from_utf8(std::mem::take(&mut self.buf))
                    .map_err(|_| Event::message(500, "Invalid UTF-8"))?,
            ));
    ...
    fn push_token(&mut self, token: Token) -> Result<(), Event> {
        ...
        if self.command_size > self.max_command_size {
            return Err(Event::message(500, "Request too long"));
        }
                ...
                State::Request { in_addr } => match ch {
                            ...
                            .as_str()
                        {
                            ...
                            "mail" => {
                                    ...
                                    && matches!(tokens.next(), Some(Token::Lt))
                                {
                                    ...
                                    Err(Event::message(500, "Invalid MAIL FROM syntax."))
                                }
                            }
                            "rcpt" => {
                                    ...
                                    && matches!(tokens.next(), Some(Token::Lt))
                                {
                                    ...
                                    Err(Event::message(500, "Invalid RCPT TO syntax."))
                                }
                            }
                            "data" => {
                            ...
                            "bdat" => {
                                let chunk_size = tokens
                                    ...
                                    .parse::<usize>()
                                    .map_err(|_| {
                                        Event::message(500, "Failed to parse chunk size.")
                                    })?;
                                let is_last = tokens
                                    .next()
                                    .and_then(|t| t.unwrap_text())
                            ...
                            "vrfy" => Ok(Request::Vrfy {
                                mailbox: tokens.next().and_then(|t| t.unwrap_text()).ok_or_else(
                                    || Event::message(500, "EXPN requires a valid text argument."),
                                )?,
                            }),
                    ...
                    _ => {
                        if !ch.is_ascii_whitespace() {
                            self.command_size += 1;
                            if self.command_size > self.max_command_size {
                                return Err(Event::message(500, "Request is too long."));
                            }
                            self.buf
    ...
    fn parse_params(&self, tokens: &mut Peekable<IntoIter<Token>>) -> Result<Vec<Param>, Event> {
        ...
        while let Some(param_name) = tokens.next() {
            let param_name = param_name
                .unwrap_text()
                .ok_or_else(|| Event::message(500, "Parameter name must be a text value."))?;
            if !matches!(tokens.next(), Some(Token::Eq)) {
                return Err(Event::message(500, "Missing '=' after parameter name."));
            }
            let param_value = match tokens.next() {
                Some(Token::Text(text)) => text,
                Some(_) => {
                    return Err(Event::message(500, "Parameter value must be a text value."));
                }
                None => return Err(Event::message(500, "Missing parameter value.")),
            };
            ...
            match param_name.as_str() {
                ...
                "size" => {
                    let size = param_value
                        .parse()
                        .map_err(|_| Event::message(500, "Size parameter must be a number."))?;
...
impl Event {
    pub fn message(code: u16, message: impl Into<Cow<'static, str>>) -> Self {
        Event::Message {
...
#[cfg(test)]
mod tests {
    use crate::parser::{Event, Response};
